...
for(idx in c(3)){ # 1,2,
	...
	I = out_equm[nrow(out_equm)-1, (numpatch+2):ncol(out_equm)] - out_equm[nrow(out_equm)-2, (numpatch+2):ncol(out_equm)]

	plot_df = data.frame(read_inc = ivector_upa, equ_inc = c(t(I)))

	saveRDS(plot_df, paste0("analysis/real_equ_inc_df_b", b[idx], "_c", c[idx], ".rds"))

	png(paste0("Figures/real_equ_inc_b", b[idx], "_c", c[idx], ".png"),
		width = 2000, height = 2000, res = 250)

	ggplot(plot_df, aes(x = read_inc, y = equ_inc)) +
	  geom_point(alpha = 0.5) +
	  xlab("real incidence data") + ylab("equilibrium incidence (C(t+1)- Ct)") +
	  theme_light()
