...
malaria.ode.fast.X_zero <- function(t,x,params) {
...
rootfun <- function (t,x,params) {
  dstate <- unlist(malaria.ode.fast.X_zero(t,x,params))[1:params[["numpatch"]]] # rate of change vector, only select prevalence
...
sse.fn <- function(p, data, xstart,
  ...
  paras <- list(numpatch = numpatch,
                ...
                H = data$H,
                m = p)
  out = as.data.frame(lsodar(xstart, 1:(5*10^5), malaria.ode.fast.X_zero, paras, rootfun=rootfun))
...
for(idx in c(3)){
	...
	if(!file.exists(paste0("analysis/out_equilibrium_union_analitical_m_Xzero_b", b[idx], "_c", c[idx], ".rds"))){
	  out_equm = as.data.frame(ode(xstart, 1:(5*10^5), malaria.ode.fast.X_zero, paras, rootfun=rootfun))
	  saveRDS(out_equm, paste0("analysis/out_equilibrium_union_analitical_m_Xzero_b", b[idx], "_c", c[idx], ".rds"))
	} else  out_equm = readRDS(paste0("analysis/out_equilibrium_union_analitical_m_Xzero_b", b[idx], "_c", c[idx], ".rds"))
	...
	# 	xequm_temp = xequm
	# 	xequm_temp[i] = 0
	# 	out_temp = ode(xequm_temp, 1:(5*10^5), malaria.ode.fast.X_zero, paras_temp, rootfun=rootfun)
	...
	saveRDS(I_chang_m, paste0("analysis/I_X_zero_union_b", b[idx], "_c", c[idx], ".rds"))
}
